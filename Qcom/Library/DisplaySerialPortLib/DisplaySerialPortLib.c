...
RETURN_STATUS
EFIAPI
DisplaySerialPortLibInitialize (
  VOID
  )
{
  VOID                *Hob;
  ...
  // try to get position hob
  Hob = GetFirstGuidHob (&gQcomFbConStateGuid);
  if (Hob != NULL && GET_GUID_HOB_DATA_SIZE (Hob) == sizeof *cur_pos) {
    cur_pos = GET_GUID_HOB_DATA (Hob);
